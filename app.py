...
.metric-card { background: linear-gradient(135deg,#1e1e2e,#2a2a3e); border:1px solid #3a3a5c; border-radius:12px; padding:20px; text-align:center; color:white; }
        ...
        with tab_in:
            ...
            if st.button("Sign In", use_container_width=True, type="primary"):
                ...
                if u in users and verify_pw(p, users[u]["password_hash"]):
                    st.session_state.update({"logged_in": True, "client_name": users[u]["name"], "username": u})
                    st.rerun()
                else:
...
with st.sidebar:
    ...
    st.title("Churn Predictor"); st.caption("E-Commerce ML Dashboard"); st.divider()
    st.markdown(f"👋 Welcome, **{st.session_state['client_name']}**")
    if st.button("🚪 Logout"):
        st.session_state.update({"logged_in": False, "client_name": "", "username": ""}); st.rerun()
    st.divider()
    st.markdown("### 📂 Data Source")
    data_source = st.radio("", ["Use demo data", "Upload Olist CSVs", "Upload single CSV"], label_visibility="collapsed")
    uploaded_file = None; olist_files = {}
    if data_source == "Upload Olist CSVs":
        st.caption("Upload all 5 Olist CSV files:")
        for fname in ["olist_customers_dataset.csv","olist_orders_dataset.csv","olist_order_items_dataset.csv","olist_order_payments_dataset.csv","olist_order_reviews_dataset.csv"]:
            f = st.file_uploader(fname, type=["csv"], key=fname)
            if f: olist_files[fname] = f
        n = len(olist_files)
        st.info(f"{n}/5 files uploaded") if n < 5 else st.success("All 5 files ready!")
    elif data_source == "Upload single CSV":
        uploaded_file = st.file_uploader("Upload customer CSV", type=["csv"])
    st.divider(); st.markdown("### ⚙️ Prediction Threshold")
    threshold = st.slider("Churn probability cutoff", 0.1, 0.9, 0.5, 0.05)
    st.divider(); st.info("Built with **XGBoost + SHAP**.\n\nPredicts which customers are likely to churn so you can take action early.")
...
if data_source == "Upload Olist CSVs" and len(olist_files) == 5:
    ...
    if st.session_state.get("olist_cache_key") != ck:
        ...
        for fn, fo in olist_files.items():
            fo.seek(0)
            with open(os.path.join(td, fn), "wb") as out: out.write(fo.read())
        st.session_state["olist_tmpdir"] = td; st.session_state["olist_cache_key"] = ck
    df_full, metrics, artifacts = get_olist_model(ck, st.session_state["olist_tmpdir"])
else:
    df_full, metrics, artifacts = get_demo_model()

model = artifacts["model"]; explainer = artifacts["explainer"]
shap_vals = artifacts["shap_values"]; X_test = artifacts["X_test"]
...
with tab1:
    if data_source == "Upload Olist CSVs" and len(olist_files) == 5:
        df_pred = df_full.drop(columns=["churn"], errors="ignore"); st.success(f"Model retrained on {len(df_pred):,} real Olist customers ✅")
    ...
    high = (results["risk_label"]=="🔴 High").sum(); medium = (results["risk_label"]=="🟡 Medium").sum()
    low  = (results["risk_label"]=="🟢 Low").sum();  avg_p  = results["churn_probability"].mean()

    c1,c2,c3,c4,c5 = st.columns(5)
    for col,val,label,color in [(c1,len(results),"Total Customers","#7c6af7"),(c2,high,"🔴 High Risk","#ff6b6b"),(c3,medium,"🟡 Medium Risk","#ffd93d"),(c4,low,"🟢 Low Risk","#6bcb77"),(c5,f"{avg_p:.1%}","Avg Churn Prob","#7c6af7")]:
    ...
    with cc:
        ...
        fig, ax = plt.subplots(figsize=(6,3), facecolor="#1e1e2e"); ax.set_facecolor("#1e1e2e")
...
with tab2:
    ...
    mp = f2.slider("Min probability",0.0,1.0,0.0,0.05); mr = f3.selectbox("Show rows",[25,50,100,200],index=1)
    dc = ["customer_id","churn_probability","risk_label","total_orders","days_since_last_order","avg_review_score","support_tickets","total_spend"]
    filt = results[(results["risk_label"].isin(rf))&(results["churn_probability"]>=mp)].head(mr)
    st.dataframe(filt[dc].style.background_gradient(subset=["churn_probability"],cmap="RdYlGn_r").format({"churn_probability":"{:.1%}","total_spend":"${:,.0f}"}),use_container_width=True,height=450)
    st.download_button("⬇️ Download Results CSV",data=filt[dc].to_csv(index=False).encode(),file_name="churn_predictions.csv",mime="text/csv")
...
with tab3:
    ...
    with cr:
        ...
        fig,ax = plt.subplots(figsize=(5,4),facecolor="#1e1e2e"); ax.set_facecolor("#1e1e2e")
        ...
        [s.set_edgecolor("#3a3a5c") for s in ax.spines.values()]; ax.legend(facecolor="#2a2a3e",labelcolor="white")
        st.pyplot(fig); plt.close()
    with cm2:
        st.markdown('<div class="section-header">Confusion Matrix</div>', unsafe_allow_html=True)
        fig,ax = plt.subplots(figsize=(4,4),facecolor="#1e1e2e"); ax.set_facecolor("#1e1e2e")
        cmat = metrics["cm"]; im = ax.imshow(cmat,cmap="Purples")
...
with tab4:
    ...
    with cb:
        ...
        fig,ax = plt.subplots(figsize=(6,5),facecolor="#1e1e2e"); ax.set_facecolor("#1e1e2e")
    ...
    with cs:
        ...
        fig=plt.gcf(); fig.set_facecolor("#1e1e2e"); st.pyplot(fig); plt.close()
    st.divider()
    ...
    shap.waterfall_plot(shap.Explanation(values=shap_vals[idx],base_values=explainer.expected_value,data=X_test.iloc[idx].values,feature_names=FEATURE_COLS),show=False)
    fig=plt.gcf(); fig.set_facecolor("#1e1e2e"); st.pyplot(fig); plt.close()
